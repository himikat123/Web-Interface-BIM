...
export default function ModalFileViewer(props: iModalFileViewer) {
    ...
    useEffect(() => {
        fetch(props.path + props.selected)
        .then(res => res.text())
        .then((result: string) => {
        ...
        content={<>
            {(props.selected.split('.')[1] === 'png' || props.selected.split('.')[1] === 'jpg') 
                ? <div className="w-full flex justify-center items-center">
                    <img src={props.path + props.selected} alt={props.selected} />
                </div>
                : (props.selected.split('.')[1] === 'json') 
                ? <div className="w-full">
                    {fileContent && <JsonView data={JSON.parse(fileContent)} shouldExpandNode={(level) => level === 0} style={theme} />}    
                </div>
