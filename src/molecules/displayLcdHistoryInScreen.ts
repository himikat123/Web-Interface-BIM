...
export function displayLcdHistoryInScreen(ctx: CanvasRenderingContext2D, dispModel: number, 
    ...
    if(!state?.skeleton) {
        ...
        displayLcdHistoryTitle(ctx, i18n.t('indoorHistory'));
    }

    if(JSON.stringify(state?.weather) !== JSON.stringify(weather) || state?.shift !== shift) {
        displayLcdHourlyCharts(ctx, dispModel, weather, shift, 'historyIn');
        for(let i=0; i<8; i++) {
            displayLcdHourlyColumn(ctx, dispModel, weather, i, shift, 'historyIn');
        }
