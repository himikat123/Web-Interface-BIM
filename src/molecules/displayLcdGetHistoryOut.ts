...
export function displayLcdGetHistoryOut(): iHourlyWeather {
    ...
    .then((json: iHistoryOut) => {
        try {
            for(let i=0; i<24; i++) {
                ...
                history.temp[i] = parseFloat(json.feeds[i].field1);
                history.hum[i] = parseFloat(json.feeds[i].field2);
                history.pres[i] = parseFloat(json.feeds[i].field3);
            }
